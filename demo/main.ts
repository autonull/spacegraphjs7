...
try {
  ...
  const graph = SpaceGraph.create('#container', {
    ...
    edges: [
      { id: 'edge-ab', source: 'node-a', target: 'node-b', type: 'FlowEdge', data: { color: 0x00ff00, flowSpeed: 2.0 } },
      { id: 'edge-bc', source: 'node-b', target: 'node-c', type: 'CurvedEdge' },
      { id: 'edge-ca', source: 'node-c', target: 'node-a', type: 'Edge' },
      { id: 'edge-img', source: 'node-a', target: 'node-img', type: 'FlowEdge', data: { color: 0x00ffff, flowSpeed: 1.5 } }
    ]
  });
