  ...
  describe('Graph', () => {
    ...
    it('should add and retrieve nodes', () => {
      ...
      class TestNode extends Node {
        get object() { return new THREE.Object3D(); }
      }
  ...
  describe('EventSystem', () => {
    ...
    it('should batch events', async () => {
      return new Promise<void>((resolve) => {
        ...
        events.emitBatched('interaction:drag', { node: {}, position: [1, 0, 0], timestamp: Date.now() });
        events.emitBatched('interaction:drag', { node: {}, position: [2, 0, 0], timestamp: Date.now() });
        events.emitBatched('interaction:drag', { node: {}, position: [3, 0, 0], timestamp: Date.now() });
