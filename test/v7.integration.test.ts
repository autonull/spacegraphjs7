...
class TestEdge extends Edge {
  ...
  constructor(spec: any, source: Node, target: Node) {
...
describe('SpaceGraphJS v7.0 Integration', () => {
  ...
  describe('Graph Operations', () => {
    it('should create graph and load from spec', () => {
      const spec = {
        nodes: [
          { id: 'node1', type: 'TestNode', position: [0, 0, 0] as [number, number, number] },
          { id: 'node2', type: 'TestNode', position: [100, 0, 0] as [number, number, number] }
        ],
        edges: [
          { id: 'edge1', type: 'TestEdge', source: 'node1', target: 'node2' }
        ]
      };
    ...
    it('should remove connected edges when removing node', () => {
      graph.fromJSON({
        nodes: [
          ...
          { id: 'n2', type: 'TestNode', position: [100, 0, 0] as [number, number, number] }
        ],
        edges: [
  ...
  describe('Plugin System', () => {
    ...
    it('should call plugin hooks', async () => {
      ...
      const testPlugin = {
        ...
        version: '1.0.0',
        init: () => {},
        onPreFrame: () => { preFrameCalled = true; }
      };
  ...
  describe('Force Layout', () => {
    it('should apply force-directed layout', async () => {
      ...
      graph.fromJSON({
        nodes: [
          { id: 'n1', type: 'TestNode', position: [0, 0, 0] as [number, number, number] },
          { id: 'n2', type: 'TestNode', position: [10, 0, 0] as [number, number, number] },
          { id: 'n3', type: 'TestNode', position: [20, 0, 0] as [number, number, number] }
        ],
        edges: [
          { id: 'e1', type: 'TestEdge', source: 'n1', target: 'n2' },
