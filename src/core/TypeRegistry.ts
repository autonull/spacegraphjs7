...
export class TypeRegistry {
    ...
    registerNode(typeName: string, constructor: NodeConstructor): void { if (!this.nodeTypes.has(typeName)) this.nodeTypes.set(typeName, constructor); }
    registerEdge(typeName: string, constructor: EdgeConstructor): void { if (!this.edgeTypes.has(typeName)) this.edgeTypes.set(typeName, constructor); }
    ...
    createNode(sgOrSpec: SpaceGraph | NodeSpec, maybeSpec?: NodeSpec): Node {
        ...
        const spec = isSpecFirst ? sgOrSpec as NodeSpec : maybeSpec!;
        ...
        return new ctor(sg ?? Object.create(null) as SpaceGraph, spec);
    }
    ...
    createEdge(sgOrSpec: SpaceGraph | EdgeSpec, specOrSource: EdgeSpec | Node, sourceOrTarget?: Node, targetOrNothing?: Node): Edge {
        ...
        const ctor = this.getEdgeConstructor(spec.type ?? 'Edge');
        if (!ctor) throw new Error(`TypeRegistry: Unknown edge type "${spec.type}". Registered: [${this.getEdgeTypes().join(', ')}]`);
        return new ctor(sg ?? Object.create(null) as SpaceGraph, spec, source, target);
    }
