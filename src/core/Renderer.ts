...
export class Renderer {
    ...
    constructor(sg: SpaceGraph, container: HTMLElement, options: RenderOptions = {}) {
        ...
        this.renderer = new THREE.WebGLRenderer({ antialias: options.antialias ?? true, alpha: options.alpha ?? true, preserveDrawingBuffer: true });
    ...
    beginFrameOptimization(timestamp: number): void {
        if (this.lastOptTime === 0) { this.lastOptTime = timestamp; return; }
        const delta = timestamp - this.lastOptTime;
        this.lastOptTime = timestamp;
        this.optFrames++;
        this.timeSinceLastOptCheck += delta;
        if (this.timeSinceLastOptCheck >= OPTIMIZER_CHECK_INTERVAL_MS) { this.fps = (this.optFrames * 1000) / this.timeSinceLastOptCheck; this.optFrames = 0; this.timeSinceLastOptCheck = 0; }
    }
    ...
    updateCulling(): void {
        ...
        for (const node of this.sg.graph.nodes.values()) {
            ...
            if (node instanceof DOMNode) node.setVisibility(inFrustum);
            else node.object.visible = inFrustum;
        }
    }
    ...
    async exportPNG(scale = 1): Promise<Blob> {
        ...
        return new Promise((resolve, reject) => {
            this.renderer.domElement.toBlob((blob) => { if (blob) resolve(blob); else reject(new Error('toBlob failed')); }, 'image/png', scale);
        });
    }
