...
export type UnionToIntersection<U> = (U extends any ? (k: U) => void extends (k: infer I) => void ? I : never : never) extends (k: infer I) => void ? I : never;
export type PickRequired<T, K extends keyof T> = Omit<T, K> & Required<Pick<T, K>>;
export type NonNullable<T> = T extends null | undefined ? never : T;
export type ExcludeNull<T> = { [K in keyof T]: Exclude<T[K], null> };
export type NonNullableKeys<T> = { [K in keyof T]: T[K] extends null | undefined ? never : K }[keyof T];
export type NullableKeys<T> = { [K in keyof T]: T[K] extends null | undefined ? K : never }[keyof T];
...
export type Listener<K extends string> = K extends `${string}:${string}` ? string : never;
export type EventMapKeys<T> = T extends Record<string, unknown> ? keyof T : never;
...
export type BiMapper<T, U, R> = (a: T, b: U) => R;
...
export type ShapeNodeData = WithSize<WithColorable<WithOpacity<BaseNodeData>>> & {
    ...
    wireframe?: boolean; transparent?: boolean; side?: 'front' | 'back' | 'double';
};

export type HtmlNodeData = WithDimensions<BaseNodeData> & {
    html?: string; className?: string; pointerEvents?: 'none' | 'auto'; labelLod?: LabelLodLevel[];
};
export type ImageNodeData = WithDimensions<WithOpacity<BaseNodeData>> & { url?: string; };
export type GroupNodeData = WithDimensions<WithColorable<WithOpacity<BaseNodeData>>> & { depth?: number; wireframe?: boolean; title?: string; };
export type NoteNodeData = WithDimensions<WithColorable<BaseNodeData>> & { text?: string; textColor?: string | number; fontSize?: number; };
export type CanvasNodeData = WithDimensions<BaseNodeData> & { dpi?: number; };
export type TextMeshNodeData = WithColorable<BaseNodeData> & {
    text?: string; fontUrl?: string; size?: number; height?: number; align?: 'left' | 'center' | 'right';
};
export type DataNodeData = WithThemable<BaseNodeData> & { data?: unknown; expanded?: boolean; maxHeight?: number; };
export type VideoNodeData = WithDimensions<BaseNodeData> & { url?: string; autoplay?: boolean; loop?: boolean; muted?: boolean; };
...
export interface AudioNodeData extends BaseNodeData, Colorable { src?: string; autoplay?: boolean; loop?: boolean; }
export type MathNodeData = WithDimensions<WithColorable<BaseNodeData>> & { math?: string; fontSize?: number; };
export interface ProcessNodeData extends BaseNodeData, Dimensions { pid?: string | number; name?: string; cpu?: number; memory?: number; }
export interface CodeEditorNodeData extends BaseNodeData, Dimensions { code?: string; language?: string; theme?: string; }
export type InstancedShapeNodeData = WithSize<WithColorable<WithOpacity<BaseNodeData>>> & { shape?: 'box' | 'sphere' | 'circle' | 'plane'; };

export type NodeData = ShapeNodeData | HtmlNodeData | ImageNodeData | GroupNodeData | NoteNodeData | CanvasNodeData | TextMeshNodeData
    | DataNodeData | VideoNodeData | IFrameNodeData | ChartNodeData | MarkdownNodeData | GlobeNodeData | SceneNodeData
    | AudioNodeData | MathNodeData | ProcessNodeData | CodeEditorNodeData | InstancedShapeNodeData;
